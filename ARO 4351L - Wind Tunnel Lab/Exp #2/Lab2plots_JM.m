...
xVals = [0 0.1 0.2 0.4 0.6 0.8 1.2 1.6 2 2.4 3.2 4 4.8 5.6 6.4 7.2 7.6 8 ...
    7.6 7.2 6.4 5.6 3.2 2.4 2 1.6 1.2 0.8 0.6 0.4 0.2 0.1 0];
...
ylim([min(yVals)-1 max(yVals)+0.5])
grid on
...
yOverc = [0	-0.01894	-0.02615	-0.03555	-0.042	-0.04683	-0.05345...
    -0.05737	-0.05941	-0.06002	-0.05803	-0.05294	-0.04563	-0.03664...
    -0.02623	-0.01448	-0.00807	0.0126	0.00807	0.01448	0.02623	0.03664	0.05803...
    0.06002	0.05941	0.05737	0.05345	0.04683	0.042	0.03555	0.02615	0.01894];
...
legend({'Uninterpolated', 'Interpolated', '$P_{lower}$', '$P_{upper}$'}, 'Interpreter', 'latex');
hold off
...
set(gca, 'YDir','reverse')
legend({'$C_{p,lower}$', '$C_{p,upper}$'}, 'Interpreter', 'latex');
xlabel('X/c (in)')
ylabel('C_p')
...
figure(3)
plot(Cp_lower, yOverc(1:17), 'linewidth', 1)
hold on
plot(Cp_upper, yOverc(17:32), 'linewidth', 1)
legend('$C_{p,lower}$', '$C_{p,upper}$', 'Interpreter', 'latex');
xlabel('C_p (in)')
ylabel('y/c')
title('-4 Deg. AOA C_p vs. y/c')
grid on
hold off
...
% Solve for C_c
% N_l_cc = trapz(Cp_lower,yOverc(1:17))
% N_u_cc = trapz(Cp_upper,yOverc(17:32))
N_l_cc = trapz(yOverc(1:17),Cp_lower)
N_u_cc = trapz(yOverc(17:32),Cp_upper)
C_N_neg4_cc = N_l_cc+N_u_cc
...
% Solve for C_c
% N_l_cc = rapz(Cp_lower,yOverc(1:17))
% N_u_cc = trapz(Cp_upper,yOverc(17:32))
N_l_cc = trapz(yOverc(1:17),Cp_lower)
N_u_cc = trapz(yOverc(17:32),Cp_upper)
C_N_13_cc = N_l_cc+N_u_cc
...
Cc = trapz(Cp_lower,yOverc(1:17)) + trapz(Cp_upper,yOverc(17:32))

AoA = [ -4 0 4 8 10 12 13 14 15 16 17 18];

%Initialize a 1x12 Matrix for C_D
for i = 1:12
    C_D(i) = Cc*cosd(AoA(i)) + C_N(i)*sind(AoA(i));
end

figure(6)
hold on
plot(AoA, C_D, 'linewidth', 1)
xlabel('AoA (deg)')
ylabel('C_D')
title('Coefficient of Drag (C_D) vs. AoA(alpha)')
legend('V = 180 fps','Location','Northwest')
grid on
hold off

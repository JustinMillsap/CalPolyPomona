...
% Set Up Density and Dimensions
rho = 1000;    %[kg/m^3] Density
l   = 30E-2;   %[m] Side Length
...
nexttile(2),hold on,grid on
    ...
    xlabel('t'); ylabel('$\phi$ [deg]','Interpreter','Latex')
    ytickformat('%.0f')
nexttile(5),hold on,grid on
    plot(t,theta,'linewidth',2,'Color',[1,0.6,0])
    xlabel('t'); ylabel('$\theta$ [deg]','Interpreter','Latex')
    ytickformat('%.0f')
nexttile(8),hold on,grid on
    plot(t,psi,'linewidth',2,'Color',[0,0.8,0])
    xlabel('t'); ylabel('$\psi$ [deg]','Interpreter','Latex')
    ytickformat('%.0f')
% Angular Velocity in B-RF coords.
nexttile(3),hold on,grid on
    plot(t,p*180/pi,'linewidth',2,'Color',[0,0.9,0.9])
     xlabel('t'); ylabel('$p$ [deg/s]','Interpreter','Latex')
    ytickformat('%.1f');
nexttile(6),hold on,grid on
    plot(t,q*180/pi,'linewidth',2,'Color',[0,0.6,1])
    xlabel('t'); ylabel('$q$ [deg/s]','Interpreter','Latex')
    ytickformat('%.1f');
nexttile(9),hold on,grid on
    plot(t,r*180/pi,'linewidth',2,'Color',[0,0,1])
    xlabel('t'); ylabel('$r$ [deg/s]','Interpreter','Latex')
    ytickformat('%.1f');
nexttile(12),hold on,grid on
    plot(t,norm_w*180/pi,'linewidth',2,'Color',[1 0 0.95])
...
function dX = func_DYNAMICS(t,X,I_B)
    ...
    % External Moments
    M_B = 1E-2*[-6*p,0,-5*r]'; % Part (a)
    M_B = 1E-2*[-5,0,0]';       % Part (b)
...
function q = R_to_Quaternions(R)
    ...
    if q0sq==max([q0sq,q1sq,q2sq,q3sq])
        ...
        q2 = 1/(4*q0)*(R(3,1)-R(1,3));
        q3 = 1/(4*q0)*(R(1,2)-R(2,1));
    elseif q1sq==max([q0sq,q1sq,q2sq,q3sq])
        q1 = sqrt(q1sq);
        q0 = 1/(4*q1)*(R(2,3)-R(3,2));
        q2 = 1/(4*q1)*(R(1,2)+R(2,1));
        q3 = 1/(4*q1)*(R(3,1)+R(1,3));
    elseif q2sq==max([q0sq,q1sq,q2sq,q3sq])
        q2 = sqrt(q2sq);
        q0 = 1/(4*q2)*(R(3,1)-R(1,3));
        q1 = 1/(4*q2)*(R(1,2)+R(2,1));
        q3 = 1/(4*q2)*(R(2,3)+R(3,2));
    else
        q3 = sqrt(q3sq);
        q0 = 1/(4*q3)*(R(1,2)-R(2,1));
        q1 = 1/(4*q3)*(R(3,1)+R(1,3));
